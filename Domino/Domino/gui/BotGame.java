...
public class BotGame extends JPanel{
		...
		boolean gameOverP1 = false;
		boolean gameOverP2 = false;
		...
		int scoreP2 = 0;
        int x,y;
        public BotGame() throws FileNotFoundException{
           ...
           JFrame dominoGame = new JFrame();
			...
			commencer.addActionListener((ActionEvent e) -> {
        			var j1 = JOptionPane.showInputDialog("Quel est ton nom ? (8 caractères conseillés)");
					...
					if(deP1 > deP2){
						...
						g.drawString(player1.name+" est à gauche et l'ordinateur est à droite.", width/4, 300);
						...
						g.setColor(Color.white);
						g.setFont(g.getFont().deriveFont(40f));
						g.drawString("L'ordinateur commence", width/3+100, 200);
						g.drawString(player1.name+" est à gauche et l'ordinateur est à droite.", width/4, 300);
						g.drawString("Si vous ne pouvez plus piocher,cela signifie que", width/4, 600);
						g.drawString("le sac de dominos est vide.", width/4, 650);
						g.drawString("Appuyer sur 'Pioche' pour commencer à jouer.", width/4, 750);
            		...
            		if(!sac.isEmpty()) {
						...
						if(Domino.getDominoPlayer(currentPlayer)!=null) {
						 ...
						 if(currentPlayer == player1) {
				        		...
				        		conteneur.add(d1);
								currentPlayer = player2;
								Domino d2=new Domino((Bot)player2);
								conteneur.repaint();
								conteneur.add(d2);
							}
	            		}
		...
		public void afficherJoueurCourant() {
        	...
        	joueur.setBounds((width/2)-150, 30, 1000, 100);
        	...
        	nameJ1.setBounds(100, 10, 400, 100);
        	...
        	nameJ2.setBounds(width-200, 10, 400, 100);
        	conteneur.add(joueur);
	    ...
	    public void mouseMoved(MouseEvent e) {
	    	if(etat) {
				int x=e.getXOnScreen()-BotGame.this.getX()-BotGame.this.getInsets().left-xClick;
				int y=e.getYOnScreen()-BotGame.this.getY()-BotGame.this.getInsets().top-yClick;				
				setLocation(x, y);		
			}
