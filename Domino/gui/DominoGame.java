...
public class DominoGame extends JPanel{
	  ...
	  File file = new File("ressources/grey.jpg");
		...
		public void removeDomino() {
			for(Component c:conteneur.getComponents()) {
				if(c instanceof Domino) {
					conteneur.remove(c);
				}
			}
		}
	    		...
	    		if(!etat) {
					...
					if(t.positionValide(plateau, i, j)) {
						...
						currentPlayer.courant=null;
							...
							if(player==player1) {
								if(player2.courant==null && sac.isEmpty()) {
									gameOver(DominoGame.this.getGraphics());
								...
								if(player1.courant==null && sac.isEmpty()) {
									gameOver(DominoGame.this.getGraphics());
